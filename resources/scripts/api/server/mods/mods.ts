...
export const searchModpacks = async (
    ...
    const { data } = await http.get(`/api/client/servers/${uuid}/mods/modpacks/search`, {
        params: { provider, query, offset, sort },
    });
...
export const previewModpack = async (
    ...
    const { data } = await http.get(`/api/client/servers/${uuid}/mods/modpacks/preview`, {
        params: { provider, project_id: projectId },
    });
    return data;
};
