...
- **`confirm` is still blocking.** Approving a destructive action re-enters the loop over the non-streaming endpoint, so a confirmed action can sit for up to the client's timeout with no progress — the problem streaming solves for `messages`, one click later.
