...
- **History windowing** (`buildProviderMessages()`): take the newest `history_limit` messages, then drop from the front until the window starts on a `user` message, because an assistant turn separated from its tool results is invalid input. If that empties the window (one turn called more tools than the limit), the whole turn is replayed instead.
