...
class DatapackZipService
{
    ...
    /**
     * ZIP files present in /datapacks that are not tracked in server_datapacks.
    ...
    /**
     * Whether the ZIP on the server contains a parseable pack.mcmeta.
     */
    public function hasPackMcmeta(Server $server, string $fileName): bool
    {
    ...
    public function parsePackMcmeta(Server $server, string $fileName, int $size): array
    {
        ...
        try {
            ...
            $result = [
                'slug' => strtolower(preg_replace('/[^A-Za-z0-9_.-]+/', '-', (string) ($meta['slug'] ?? $fallback['slug']))),
