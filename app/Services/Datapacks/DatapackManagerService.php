...
use App\Services\Datapacks\CurseForgeService;
use App\Services\Datapacks\ModrinthService;
...
class DatapackManagerService
{
    ...
    public function __construct(
        ...
        \App\Services\Datapacks\ModrinthService $modrinth,
        \App\Services\Datapacks\CurseForgeService $curseForge,
    ) {
        $this->providers = [
            \App\Services\Datapacks\ModrinthService::PROVIDER => $modrinth,
            \App\Services\Datapacks\CurseForgeService::PROVIDER => $curseForge,
        ];
    }
    ...
    /**
     * Same datapack already installed from a different provider, matched by slug/title.
     * For manual datapacks, also checks against zip metadata to catch filename mismatches.
     */
    public function crossProviderDuplicate(Server $server, string $providerName, string $slug): ?ServerDatapack
    {
        ...
        foreach ($server->datapacks as $d) {
            if ($d->provider === $providerName) {
                continue;
            }

            // Check slug and title
            if (strtolower($d->slug) === $normalized || strtolower($d->title) === $normalized) {
                return $d;
            }

            // For manual datapacks, extract zip metadata and compare
            if ($d->provider === 'manual') {
                try {
                    $zipService = app(DatapackZipService::class);
                    $meta = $zipService->parsePackMcmeta($server, $d->file_name, 0);
    ...
    /**
     * Install a datapack onto the server.
     */
    public function install(Server $server, string $providerName, string $projectId, ?string $title = null, ?string $iconUrl = null, ?string $versionId = null, ?string $slug = null): ServerDatapack
    {
        ...
        $version = $versionId
            ? collect($provider->versions($projectId, $gameVersion ? [$gameVersion] : [], 100))
                ->firstWhere('id', $versionId)
            : $provider->resolveVersion($projectId, $gameVersion ? [$gameVersion] : []);
    ...
    public function update(Server $server, ServerDatapack $datapack): ServerDatapack
    {
        $latest = $this->provider($datapack->provider)->latestVersion(
            $datapack->project_id,
            $datapack->version_id,
            $this->gameVersion($server) ? [$this->gameVersion($server)] : []
        );
