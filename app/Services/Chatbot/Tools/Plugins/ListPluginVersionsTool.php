...
class ListPluginVersionsTool extends PluginTool
{
    public function name(): string
    {
        return 'list_plugin_versions';
    }

    public function description(): string
    {
        return 'List the downloadable versions of one plugin, newest first, filtered to those the registry marks as compatible with this server\'s loader and game version. Each entry has a version_id you can pass to install_plugin to install that exact build. Use this whenever the newest release does not suit — for example when the server runs an older game version, or when a plugin needs rolling back to the build that last worked. An empty list means the registry publishes nothing compatible with this server, not that the plugin has only one version.';
    }
    ...
    private function note(?string $gameVersion, bool $empty): string
    {
        ...
        return $empty
            ? "The registry lists no version of this plugin compatible with game version $gameVersion on this server's loader."
            : 'Pass a version_id to install_plugin to install that exact build.';
    }
}
