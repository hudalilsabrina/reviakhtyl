...
trait ManagesChatbotTurns
{
    ...
    /**
     * The single-model tool-calling loop, used when orchestration is off and as
     * the "answer directly" path inside orchestration: a simple request is
     * served by one capable model holding every tool, not by delegation.
     *
     * @return Collection<int, ChatbotMessage>
     */
    protected function runFlatLoop(ChatbotConversation $conversation, ?callable $emit = null): Collection
    {
    ...
    /**
     * Rebuilds the provider-shaped message list from stored history, using the
     * flat assistant's system prompt.
     *
