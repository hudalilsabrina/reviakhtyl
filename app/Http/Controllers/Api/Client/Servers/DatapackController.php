...
class DatapackController extends ClientApiController
{
    ...
    public function versions(SearchDatapacksRequest $request, Server $server): array
    {
        ...
        $gameVersion = $this->manager->gameVersion($server);
        $versions = $this->manager->provider($request->input('provider', 'modrinth'))->versions(
    ...
    private function assertCleanZipScan(Server $server, string $remotePath): void
    {
        ...
        $scan = $scanService->scanRemoteFile($fileRepository, $server, $remotePath);

        if ($scan->isInfected() || ($scan->isError() && $scanService->isStrict())
            || ($scan->isError() && str_contains((string) $scan->getMessage(), 'Failed to fetch remote file'))) {
            $fileRepository->setServer($server)->deleteFiles(dirname($remotePath), [basename($remotePath)]);
